import { describe, expect, test, vi } from 'vitest'
import { createSpringSystem } from '../src/index'

describe('Spring: input validation', () => {
  test('throws when mass is 0', () => {
    const system = createSpringSystem()
    expect(() => system.createSpring({ mass: 0, tension: 1, damping: 1 })).toThrow(
      'Mass must be greater than 0',
    )
  })

  test('throws when mass is negative', () => {
    const system = createSpringSystem()
    expect(() => system.createSpring({ mass: -1, tension: 1, damping: 1 })).toThrow(
      'Mass must be greater than 0',
    )
  })

  test('throws when tension is 0', () => {
    const system = createSpringSystem()
    expect(() => system.createSpring({ mass: 1, tension: 0, damping: 1 })).toThrow(
      'Tension must be greater than 0',
    )
  })

  test('throws when damping is negative', () => {
    const system = createSpringSystem()
    expect(() => system.createSpring({ mass: 1, tension: 1, damping: -1 })).toThrow(
      'Damping must be greater than or equal to 0',
    )
  })

  test('allows damping of 0', () => {
    const system = createSpringSystem()
    expect(() => system.createSpring({ mass: 1, tension: 1, damping: 0 })).not.toThrow()
  })

  test('throws when precision is 0', () => {
    const system = createSpringSystem()
    expect(() =>
      system.createSpring({ mass: 1, tension: 1, damping: 1, precision: 0 }),
    ).toThrow('Precision must be greater than 0')
  })

  test('throws on setting invalid mass at runtime', () => {
    const system = createSpringSystem()
    const spring = system.createSpring({ mass: 1, tension: 1, damping: 1 })
    expect(() => { spring.mass = 0 }).toThrow('Mass must be greater than 0')
  })

  test('throws on setting invalid tension at runtime', () => {
    const system = createSpringSystem()
    const spring = system.createSpring({ mass: 1, tension: 1, damping: 1 })
    expect(() => { spring.tension = -1 }).toThrow('Tension must be greater than 0')
  })

  test('throws on setting invalid damping at runtime', () => {
    const system = createSpringSystem()
    const spring = system.createSpring({ mass: 1, tension: 1, damping: 1 })
    expect(() => { spring.damping = -1 }).toThrow('Damping must be greater than or equal to 0')
  })

  test('throws on setting invalid precision at runtime', () => {
    const system = createSpringSystem()
    const spring = system.createSpring({ mass: 1, tension: 1, damping: 1 })
    expect(() => { spring.precision = 0 }).toThrow('Precision must be greater than 0')
  })
})

describe('Spring: default values', () => {
  test('target defaults to 0 when neither target nor value is set', () => {
    const system = createSpringSystem()
    const spring = system.createSpring({ mass: 1, tension: 1, damping: 1 })
    expect(spring.target).toBe(0)
    expect(spring.value).toBe(0)
  })

  test('target defaults to value when only value is set', () => {
    const system = createSpringSystem()
    const spring = system.createSpring({ mass: 1, tension: 1, damping: 1, value: 50 })
    expect(spring.target).toBe(50)
    expect(spring.value).toBe(50)
  })

  test('value defaults to target when only target is set', () => {
    const system = createSpringSystem()
    const spring = system.createSpring({ mass: 1, tension: 1, damping: 1, target: 50 })
    expect(spring.target).toBe(50)
    expect(spring.value).toBe(50)
  })

  test('precision defaults to 2', () => {
    const system = createSpringSystem()
    const spring = system.createSpring({ mass: 1, tension: 1, damping: 1 })
    expect(spring.precision).toBe(2)
  })
})

describe('Spring: jumpTo', () => {
  test('instantly moves value to new position', () => {
    const system = createSpringSystem()
    const spring = system.createSpring({
      mass: 1, tension: 170, damping: 26, target: 0, value: 0,
    })

    spring.jumpTo(100)
    expect(spring.value).toBe(100)
    expect(spring.target).toBe(100)
  })

  test('resets velocity to zero', () => {
    const system = createSpringSystem()
    const spring = system.createSpring({
      mass: 1, tension: 170, damping: 10, target: 100, value: 0,
    })

    // Tick to build up velocity
    system.tick(0.1)
    expect(spring.velocity).not.toBe(0)

    spring.jumpTo(50)
    expect(spring.velocity).toBe(0)
  })

  test('spring is resting after jumpTo', () => {
    const system = createSpringSystem()
    const spring = system.createSpring({
      mass: 1, tension: 170, damping: 10, target: 100, value: 0,
    })

    spring.jumpTo(50)
    expect(spring.resting).toBe(true)
  })
})

describe('Spring: events', () => {
  test('onUpdate fires on each tick while active', () => {
    const system = createSpringSystem()
    const spring = system.createSpring({
      mass: 1, tension: 170, damping: 10, target: 0, value: 100,
    })

    const onUpdate = vi.fn()
    spring.onUpdate(onUpdate)

    system.tick(1 / 60)
    system.tick(1 / 60)
    system.tick(1 / 60)

    expect(onUpdate).toHaveBeenCalledTimes(3)
  })

  test('onStart fires when spring begins moving', () => {
    const system = createSpringSystem()
    const spring = system.createSpring({
      mass: 1, tension: 170, damping: 10, target: 0, value: 0,
    })

    const onStart = vi.fn()
    spring.onStart(onStart)

    // Spring is resting — no start event on tick
    system.tick(1 / 60)
    expect(onStart).not.toHaveBeenCalled()

    // Change target to trigger motion
    spring.target = 100
    expect(onStart).toHaveBeenCalledOnce()
  })

  test('onStop fires when spring comes to rest', () => {
    const system = createSpringSystem()
    const spring = system.createSpring({
      mass: 1, tension: 170, damping: 26, target: 0, value: 1,
    })

    const onStop = vi.fn()
    spring.onStop(onStop)

    // Simulate until resting
    for (let i = 0; i < 600; i++) {
      system.tick(1 / 60)
      if (spring.resting) break
    }

    expect(onStop).toHaveBeenCalled()
  })

  test('unsubscribe function works for onUpdate', () => {
    const system = createSpringSystem()
    const spring = system.createSpring({
      mass: 1, tension: 170, damping: 10, target: 0, value: 100,
    })

    const onUpdate = vi.fn()
    const unsub = spring.onUpdate(onUpdate)

    system.tick(1 / 60)
    expect(onUpdate).toHaveBeenCalledOnce()

    unsub()
    system.tick(1 / 60)
    expect(onUpdate).toHaveBeenCalledOnce()
  })
})

describe('Spring: parameter changes mid-animation', () => {
  test('changing mass mid-animation does not crash', () => {
    const system = createSpringSystem()
    const spring = system.createSpring({
      mass: 1, tension: 170, damping: 10, target: 0, value: 100,
    })

    system.tick(1 / 60)
    spring.mass = 5
    expect(() => system.tick(1 / 60)).not.toThrow()
    expect(spring.value).not.toBeNaN()
  })

  test('changing tension mid-animation does not produce NaN', () => {
    const system = createSpringSystem()
    const spring = system.createSpring({
      mass: 1, tension: 170, damping: 10, target: 0, value: 100,
    })

    system.tick(1 / 60)
    spring.tension = 300
    system.tick(1 / 60)
    expect(spring.value).not.toBeNaN()
    expect(spring.velocity).not.toBeNaN()
  })

  test('changing damping to cross solver boundary works', () => {
    const system = createSpringSystem()
    const spring = system.createSpring({
      mass: 1, tension: 170, damping: 10, target: 0, value: 100,
    })

    // Start underdamped, switch to overdamped
    system.tick(1 / 60)
    spring.damping = 40
    system.tick(1 / 60)

    expect(spring.value).not.toBeNaN()
    expect(spring.velocity).not.toBeNaN()
  })

  test('spring still converges after parameter change', () => {
    const system = createSpringSystem()
    const spring = system.createSpring({
      mass: 1, tension: 170, damping: 10, target: 0, value: 100,
    })

    // Tick for a bit, change params, then simulate to completion
    for (let i = 0; i < 30; i++) system.tick(1 / 60)
    spring.tension = 200
    spring.damping = 30

    for (let i = 0; i < 600; i++) {
      system.tick(1 / 60)
      if (spring.resting) break
    }

    expect(spring.resting).toBe(true)
    expect(spring.value).toBeCloseTo(0, 0)
  })
})

describe('Spring: dispose', () => {
  test('disposed spring no longer ticks', () => {
    const system = createSpringSystem()
    const spring = system.createSpring({
      mass: 1, tension: 170, damping: 10, target: 0, value: 100,
    })

    const onUpdate = vi.fn()
    spring.onUpdate(onUpdate)

    system.tick(1 / 60)
    expect(onUpdate).toHaveBeenCalledOnce()

    spring.dispose()
    system.tick(1 / 60)
    // onUpdate should not fire again — emitter was cleared
    expect(onUpdate).toHaveBeenCalledOnce()
  })
})
